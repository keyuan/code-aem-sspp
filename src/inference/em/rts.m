function stats = rts(stats, param)
% stats = rts_s(stats)
%
% usage:
%   RTS (fixed interval) smoother for vector stats systems
%
% input:
...
for nsamp = totsamp-1 : -1 : 1
    ...
    autoexp(:,:,nsamp)  = covsmth(:,:,nsamp) + ...
                          xsmth( :, nsamp )*xsmth(:,nsamp)';
    crosscov(:,:,nsamp) = jk*covsmth(:,:,nsamp+1);
    crossexp(:,:,nsamp) = crosscov(:,:,nsamp) + ...
                          xsmth(:,nsamp+1)*xsmth(:,nsamp)';
end
%-------------------------------------------------------------------------%
